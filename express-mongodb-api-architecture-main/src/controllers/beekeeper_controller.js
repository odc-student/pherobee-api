...
const getSubownerById = async (req, res) => {
  ...
  const subowner = await Subowner.findById(subownerId).populate([{
    path: "farmAccess",
    populate: {path: "beehives", populate: "hiveLog"}
  }]).select("-password");
...
const createSubownerAndAssignFarm = async (req, res) => {
  try {
    ...
    beekeeper.subowners.push(subowner);
    await beekeeper.save();
    // Assign the farm to the subowner
    const farm = await Farm.findById(farmId);
    if (!farm) {
      return res.status(404).json(createApiResponse({message: 'Farm not found'}, 404, 'Farm not found', false));
    }
    ...
    res.status(201).json(createApiResponse(subowner, 201, 'Subowner created and farm assigned successfully'));
  } catch (error) {
    res.status(500).json(createApiResponse(error, 500, "Error when creating the subowner and assigning the farm: " + error.message, false));
  }
};

module.exports = {
  createSubownerAndAssignFarm,
